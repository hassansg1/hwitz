          ...
          <div class="modal-content">
            ...
            <div class="modal-body text-start ">
                <div class="table-responsive mb-20">
                    <table>
                    <tr>
                        <th># <Sorting :sortColumn="'id'" /></th>
                        <th>Name <Sorting :sortColumn="'name'" /></th>
                        <th>Action <Sorting :sortColumn="'event_type'" /></th>
                        <th>Timestamp <Sorting :sortColumn="'timestamp'" /></th>
                    </tr>
                    <tr class="border_bottom" v-for="(log,index) in data" :key="index">
                        <td class="h4">{{index+1}}</td>
                        <td>{{log.name}}</td>
                        <td class="text-medium color-secondary">{{log.event_type}}</td>
  ...
  export default {
    ...
    methods: {
        loadData(pageNumber = null){
            ...
            this.$http
            .post("/get_entry_activity/"+this.id , this.params)
            .then((response) => {
