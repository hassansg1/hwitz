      ...
      <div class="row">
        <div class="col-md-12 mb-5 mb-md-0">
          <form @submit.prevent="updateUser" @keydown="form.onKeydown($event)">
            ...
            <div class="row">
              <div class="col-md-2 mb-3">
                ...
                <div>
                  <input
                      ...
                      class="form-control"
                      v-model="mobile1"
                      @blur="checkStaffMobile"
                  />
                  <span v-if="user.mobile_verification == 'No'">
                    <a
                        :href="mobileVerificationLink"
                        target="_blank"
              ...
              <div class="col-6 mb-3">
                ...
                <div>
                  ...
                  />
                  <span
                      v-if="user.email_verified == 0 || user.email_verified == ''"
                      class="red"
                  >
                    Not receive verification email yet?
                    <a
            ...
            <div
                class="row"
                v-if="users && typeof users !== 'undefined' && users.length > 0"
            >
            ...
            <div class="row">
              <div class="col-md-6 multiselect-border multiselect-bold">
...
export default {
  ...
  methods: {
    ...
    fetchUserDetails() {
      this.showLoader();
      this.$http
          .get("/fetchUserData/" + this.$route.params.userId)
          .then((response) => {
            this.user = response.data;
            this.form.fill(response.data);
            this.form.usertype = this.user.usertype
                ? this.user.usertype.id
                : "";
            this.form.building = this.user.buildings;
            const mobile = this.user.mobile.split("-");
    ...
    updateUser() {
      ...
      // this.form.building = this.form.building.map((option) => option.id);
      this.form.mobile = this.mobile1 + "-" + this.mobile2 + "-" + this.mobile3;
      this.form
          .post("/api/updateStaffUser/" + this.$route.params.userId)
          .then(({data}) => {
            Toast.fire({
              icon: "success",
              title: "Staff user updated successfully.",
            });
    ...
    checkStaffMobile() {
      ...
      let params = {
        ...
        bypass_id: this.$route.params.userId,
      };
      this.$http
